...
const cageService = {
  getAllCages: async () => {
    try {
      ...
      const data = await client.query(api.cages.list, {})
  ...
  getActiveCages: async () => {
    try {
      const client = getConvexHttpClient()
      const data = await client.query(api.cages.getActive, {})
      return { data, error: null }
    } catch (error) {
      return { data: [], error }
    }
  ...
  updateCage: async (cageId, cageData) => {
    try {
      ...
      for (const [from, to] of Object.entries(map)) {
        if (cageData[from] !== undefined) patch[to] = cageData[from]
        if (cageData[to] !== undefined) patch[to] = cageData[to]
      }
