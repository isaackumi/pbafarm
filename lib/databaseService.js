...
export const cageService = {
  ...
  getAllCages: async () => {
    ...
    return await supabase.from('cage_info').select('*').order('name')
  },

  // Get a specific cage by ID
  getCageById: async (id) => {
    await cageService.ensureTablesExist()
    return await supabase.from('cage_info').select('*').eq('id', id).single()
  },
  ...
  createCage: async (cageData) => {
    ...
    return await supabase.from('cage_info').insert([cageData]).select()
  },
  ...
  deleteCage: async (cageId) => {
    ...
    return await supabase.from('cage_info').delete().eq('id', cageId)
  },
