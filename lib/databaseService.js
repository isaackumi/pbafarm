...
export const harvestRecordService = {
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert([{
          ...
          total_weight: recordData.total_weight,
  ...
  async addSamplingData(samplingData) {
    try {
      const { data, error } = await supabase
        .from('harvest_sampling')
        .insert([{
          harvest_id: samplingData.harvest_id,
          crate_size: samplingData.crate_size,
      ...
      console.error('Error adding sampling data:', error)
      return { data: null, error }
    }
  },
  ...
  async exportHarvestRecords() {
    try {
      const { data, error } = await this.getHarvestRecords()
      if (error) throw error
      ...
      const csvData = data.map(record => ({
        ...
        'Sampling Data': record.harvest_sampling ? JSON.stringify(record.harvest_sampling) : '',
        'Notes': record.notes
      }))
