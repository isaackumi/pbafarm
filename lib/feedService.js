...
export const feedService = {
  async getAllPurchases() {
    try {
      ...
      const data = await client.query(api.feed.listPurchases, withActiveLocation())
  ...
  async createPurchase(purchaseData) {
    try {
      ...
      const id = await client.mutation(api.feed.createPurchase, {
        ...
        supplierId: purchaseData.supplier_id || purchaseData.supplierId || undefined,
  ...
  async updatePurchase(id, purchaseData) {
    try {
      ...
      await client.mutation(api.feed.updatePurchase, {
        ...
        supplierId:
          purchaseData.supplier_id || purchaseData.supplierId || undefined,
        batchNumber:
  ...
  async getFeedUsageStats(timeRange = '30d') {
    try {
      ...
      const [rows, feedTypes, cages] = await Promise.all([
        client.query(api.feed.listUsage, { dateFrom, dateTo }),
